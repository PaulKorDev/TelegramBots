...
async def start_part3(message: types.Message):
    ...
    await message.answer("А теперь вы увидите супер слайдер с кнопочками и текстом", reply_markup=ReplyKeyboardRemove())
...
async def slider_callback_prev(callback: types.CallbackQuery):
    ...
    if slider_count > 1:  
        ...
        await callback.answer("Вы куда? Это первый слайд", show_alert=True)
