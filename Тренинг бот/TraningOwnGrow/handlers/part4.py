...
#1 - Создать тест                   +
#2 - Вывести фото с изменением      +
#3 - Предложить оценку              +
#- - Создать модуль для оплаты
#4 - Предложить донат
#5 - Сделать скам тренинг           +
...
async def true_chek(callback : types.CallbackQuery):
    ...
    if count_testq == 6:
        ...
        await callback.message.answer("Теперь ставь оценку", reply_markup=marks_inkb)
...
async def mark_chek(callback : types.CallbackQuery):
    ...
    if (callback.data == 'mark_1_callback'):
        if real_mark == 1:
            ...
            real_mark = 0
        await callback.answer(text='Ты еблан? Исправь', show_alert=True)
    elif (callback.data == 'mark_2_callback'):
        if real_mark == 2:
            await sql_add_mark(1, callback.from_user.username)
            real_mark = 0
        await callback.answer(text='Этого мало, ставь больше', show_alert=True)
    elif (callback.data == 'mark_3_callback'):
        if real_mark == 3:
            await sql_add_mark(1, callback.from_user.username)
            real_mark = 0
        await callback.answer(text='Я просил оценить мой тренинг, а не свой член, добавь бал', show_alert=True)
    elif (callback.data == 'mark_4_callback'):
        if real_mark == 4:
            await sql_add_mark(1, callback.from_user.username)
            real_mark = 0
        await callback.answer(text='Ты промазал, попробуй еще раз', show_alert=True)
    elif (callback.data == 'mark_5_callback'):
        if real_mark == 5:
            await sql_add_mark(1, callback.from_user.username)
            real_mark = 0
        await callback.message.answer(text='Я рад, что вам понравился мой тренинг, еще никто его не оценивал ниже 5, это потому что он менят жизни людей.', reply_markup=kb_to_pay)
...
async def yes_donat(callback : types.CallbackQuery):
    ...
    await callback.message.answer('Будем честны, у вас нет таких денег, вы нищеброд, но я уважаю ваше желание поступить правильно, поэтому кидайте сюда, сколько у вас есть: 5469 5500 5257 8686')
